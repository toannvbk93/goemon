...
class UserModel {
  ...
  //
  public static async authenticate(email: String, password: String): Promise<UserDocument> {

      User.findOne({ email: email }).exec().then( (user) => {
        let hash = PassportUtility.getHash(password.toString());
  ...
  public static createUser(email: String, password: String): Promise<UserDocument> {
    return new Promise( (resolve, reject) => {
      let user = new User();
      user.email = email;
      user.password = PassportUtility.getHash(password.toString());
